...
export default function MovieCards({ name, id }) {
  ...
  return (
    <Link
      to={`/movie/${id}`}
      className=" w-[220px] mt-9  group bg-white  dark:bg-transparent  rounded-md hover:scale-105 transition-all duration-200 overflow-hidden flex-shrink-0   mx-7  h-[300px] "
    >
      {data?.Poster !== "N/A" ? (
        <div
